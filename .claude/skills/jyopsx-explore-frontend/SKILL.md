...
description: 前端团队专属深度探索模式。在通用 explore 全部流程基础上，新增 Figma 链接自动落盘与架构审查卡点（architect-review Agent）。
allowed-tools: Bash(openspec:*)
license: MIT
compatibility: Requires jy-openspec CLI and architect-review agent.
metadata:
  author: jy-openspec
  version: "1.0"
...
1.  **探索上下文 + Figma 链接捕获 + 截图同步**（HARD-GATE 两步原子，不可拆解）：主动检查项目文件、现有 Specs 和最近的归档记录。
    同时启动 Figma 链接监听：在整个对话过程中，一旦用户提供 figma.com 链接，**立即按以下顺序执行两步原子**（用户一次粘多条链接 → 逐条迭代处理，不批量末尾呈现）：

    ① **写 figma.md**：
       - 若当前已有活动变更 → 使用 Write 工具将链接写入 `<changeRoot>/figma.md`（`changeRoot` 取自 `jy-openspec status --change "<变更名>" --json`，不要假设仓库内相对路径），格式如下：
         \`\`\`
         # Figma 设计资源
         > 由 jyopsx-explore-frontend 在探索阶段自动记录
         ## 链接
         - [设计稿](<figma-url>)
         ## 关联变更
         - 变更名称：<name>
       ...
       - 写入/暂存后告知用户："Figma 链接已记录至 figma.md"。

    ② **拉截图（必须立即调用，截图必须进入对话上下文）**：用 `mcp__figma__get_screenshot` 工具拉取该链接对应的节点截图。
       ⚠ 若 URL 无 node-id（裸文件链接，仅 `https://www.figma.com/design/<fileKey>/<file-name>`，没有 `?node-id=...`）→ **必须主动请用户提供具体 node-id**，严禁默默跳过本步。
       ⚠ get_screenshot 返回的截图必须真正进入对话上下文，不能只让它躺在工具响应缓冲里 —— 把图嵌入到当前回复消息中，让后续推理能看到真实画面。
       截图进入上下文后，本条链接处理完毕；进入下一条（若有）。无需向用户口头总结你看到的画面，也无需等待用户的视觉理解确认。

    严禁：
    - 链接已写入 figma.md 但 `get_screenshot` 未调用。
    - `get_screenshot` 已调用但截图未进入对话上下文（图躺在工具响应缓冲里）。
    - 凭用户文字描述脑补设计稿内容。

    <HARD-GATE>
    【绝对禁令】：在所有已捕获的 figma.com 链接均完成上述 ①② 两步原子前，
    **不得**进入 Step 3（单点澄清 / 头脑风暴）。这是已被生产反复证实的失败模式：
    链接已捕获但截图未读 → Step 3 凭文字脑补 → 需求理解偏离设计稿真值。
    </HARD-GATE>
